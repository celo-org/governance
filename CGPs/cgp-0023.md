...
Following [CR3](https://github.com/celo-org/governance/blob/main/CGPs/cgp-0022.md), cEUR will be in a status where it will be impossible to mint or burn due to empty buckets that are not possible to update (because Mento buckets are zero and there are no oracles enabled to update them), but cEUR will be transferable. Freezing is meant to prevent cEUR being transferred before enabling the stability mechanism.
...
Please note this proposal shall be submitted after CR3 is proposed, because it needs an address for StableTokenEUR, but it can be executed before or after.
