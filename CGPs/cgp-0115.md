...
[
  {
    ...
    "function": "transfer",
    "args": [
      "0xf559940652F887503eAC12F951bdd73bc5B9Bf0B", #replace
...
```
The ReleaseGold contract is connected to a Timelock contract. CELO-Guild must use the Timelock contract to request Stage #2, #3, #4 disbursements. Celo Governance is configured to have the CANCELLER_ROLE in the Timelock contract and has 2 weeks to cancel Stage #2, #3, #4 disbursements to CELO-Guild.

## Verification

First, verify the on-chain proposal that 1) the destination address matchs the address of the ReleaseGold contract, **0x33020296A0Da7C897D2d6dADeF5FeCEbe2667aD7(replace)** and 2) that the amount is 10,000,000 CELO, equivalent to 10000000000000000000000000 or 1e25.

celocli governance:show --proposalID 155 --node https://forno.celo.org

Second, verify the ReleaseGold contract with parameters that match the proposal:

celocli releasecelo:show --contract **0x33020296A0Da7C897D2d6dADeF5FeCEbe2667aD7 (replace)** --node https://forno.celo.org

In particular check these parameters:

```
releaseOwner: 0xD533Ca259b330c7A88f74E000a3FaEa2d63B7972  # check this is the Timelock contract address
beneficiary: 0x8c7510C66f1BbF8f700a2665c898b3FB77550E38   # CELO-Guild Multisig cLabs
maxDistribution: 1000000000000000000000000 (~1.000e+24) # Stage1 1M CELO disbursement
```

Third, verify the Timelock contract [here](https://alfajores.celoscan.io/address/0xe118331af835bf43b0bb834a3c76c3a89f8def08#readContract) that 1) the Celo Governance (0xD533Ca259b330c7A88f74E000a3FaEa2d63B7972) has the CANCELLER_ROLE and 2) the minDelay is 1209600 (60 * 60 * 24 * 14 = 2 weeks). 

To request stage #2, #3 #4 disbursements, CELO-Guild Multisig must propose a transaction in the Timelock contract to call "function setMaxDistribution(uint256 distributionRatio) external onlyReleaseOwner" in ReleaseGold contract, with parameter distributionRatio = 300/600/1000 (30%/60%/100%) to increase maxDistribution to 3M/6M/10M CELO tokens. This transaction can't be excuted within 2 weeks due to minDelay in Timelock contract and can be cancelled by Celo Governance.  
