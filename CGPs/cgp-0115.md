...
```
The ReleaseGold contract is connected to a Timelock contract. CELO-Guild has to use the Timelock contract to request Stage #2, #3, #4 disbursements. Celo Governance is configured to have the CANCELLER_ROLE in the Timelock contract and has 2 weeks to cancel Stage #2, #3, #4 disbursements to CELO-Guild.

## Verification
Run command: celocli governance:show --proposalID 155 --node https://forno.celo.org
