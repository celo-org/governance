...
In this governance proposal, we propose to add oracles for `USDC/EUR` and `USDC/BRL`. These oracles will be used for creating `cEUR/axlUSDC` and `cREAL/axlUSDC` pools, allowing users to directly issue and redeem `cEUR` and `cREAL` against `USDC`, which will be part of the 2nd phase of the Multi-Collateral Mento upgrade (see [Mento Upgrade #1 deployment timeline](https://forum.celo.org/t/mento-upgrade-1-deployment-timeline/5219)).
The set of oracle providers proposed is the same as the oracle providers for the existing rates.

### Status

Following the Multi-Collateral Mento deployment, [CGP 71](https://github.com/celo-org/governance/blob/main/CGPs/cgp-0071.md) was executed to add oracles for the `USDC/USD` rate, which allowed the creation of the `cUSD/USDC` pool to enable direct issuance and redemption of `cUSD` against `USDC`. This CGP is a continuation of that initial effort to extend that functionality to other stable assets.
