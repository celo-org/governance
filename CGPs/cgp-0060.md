...
## Market Interference
Whenever arbitrageurs have matched cUSD/USDCe prices with the Stable Swap Pool, most users will trade with the reserve Stable Swap Pool. Due to the setup ([see Arbitrage and Dex Competition](#arbitrage-and-dex-competition) ) it has lower fee and slippage especially for larger trades. If DEX router include the Stable Swap Pool, this shift towards Stable Swap Pool will become even more likely.
...
1. We need to enable a larger scale 1:1 solution as far as possible to support cStable adoption.
2. The Stable Swap Pool is a temporary setup and interim solution.
3. The Stable Swap Pool is only affecting one pair. 
4. The setting can be changed. If we notice that with the creation of the Stable Swap Pool, the entire market on Celo becomes distorted, we can pause and propose changing parameters via governance.
