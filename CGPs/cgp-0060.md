...
## Arbitrage
The setup of the StableSwap pool is competitive. The transactions fee us on a similar level as for stable pairs on other DEXs, and additionally it uses a constant sum market making function. As a consequence, especially larger transactions will be cheaper with the constant sum StableSwap Pool.

Arbitrageurs will take advantage of the arbitrage opportunity. In the current market situation (cUSD/USDC price at ~0.995 ), arbitrageurs will buy cheaper cUSD in the market and sell it at the StableSwap Pool for a cUSD/USDC price of 1. Ignoring fees and slippage, the maximal profit they can make is

$$ 5\,000\,000 (1-0.995) = 25\,000\mathrm{\ USDCet} \ .$$

## Reserve Balance
The reserve will always be able to unwind

$$ \lambda \mathrm{USDCet} - (1-\lambda) \mathrm{cUSD}\ , \ \lambda\in[0,1]$$

independently whether arbitrageurs take arbitrage opportunities. Under the assumption that the USDC/USD price is 1 and that cUSD/USD around, the reserve balance will not change:

**Case $\lambda =1$:** All 5m pre-minted cUSD have been minted into the market and the collateral of 5m USDCet is in the USDCet pool. Half of the USDCet have been reserve owned already. The additional 5m USDCet worth 5m USD serve as collateral of the 5m pre-minted cUSD that have been minted into the market. In total the unwinding of the pool the reserve balance increases by 5m USDCet collateral for 5mm cUSD which does not change the net reserve value.  
**Case $\lambda =0$:** Additional 5m cUSD have been 'contracted' and collateral of 5m USDCet have been liquidated. 10m cUSD will be burnt. Half of the cUSD have only pre-minted and have not created 'debt'. The additional 5m cUSD remove 'debt' worth 5m USD and release 5m USDC worth 5m USD. In total the unwinding of the pool the reserve balance decreases by 5m USDCet  collateral and 5m cUSD of 'debts' which does not change the net reserve value.  
The case $\lambda <0.5$, i.e. more than 5m cUSD and less than 5m  USDCet, is similar to $\lambda=0$ and the case $\lambda >0.5$, i.e. less than 5m cUSD and more than 5m USDCet, is similar to $\lambda=1$. The only difference is that there is a remaining amount of remaining amount of pre-owned USDCet that will be returned or an remaining amount of preminted cUSD that will be burnt, respectively.
...
## Arbitrage
The setup of the StableSwap pool is competitive. It transactions fees on par with DEXs and uses a constant sum market making function. As a consequence, especially larger transactions will be cheaper with the constant sum StableSwap Pool.

Arbitrageurs will take advantage of the arbitrage opportunity. In the current market situation (cUSD/USDC price at ~0.995 ), arbitraguers will buy cheaper cUSD in the market and sell it at the StableSwap Pool for a cUSD/USDC price of 1. Ignoring fees and slippage, the maximal profit they can make is

$$ 5\,000\,000 (1-0.995) = 25\,000\mathrm{\ USDCet} \ .$$

## Reserve Balance
The reserve will always be able to unwind 
$$ \lambda \mathrm{USDCet} - (1-\lambda) \mathrm{cUSD}\ .$$
Under the assumption that the USDC/USD price is 1 and that cUSD/USD around, the reserve balance will not change.
...
There is a good chance that whenever arbitrageurs have matched the DEXs prices with the reserve, all users will only trade with the reserve cUSD/USD pool. It has lower fee+slippage especially for larger trades. If DEX router include the reserve pool, this will become even more likely.
In other words DEXs have to compete with reserve liquidity. I don’t think this is our intention.
I believe in traditional central banking injections are happening through bank as liquidity providers. This setup avoids competition between central bank and banks.
