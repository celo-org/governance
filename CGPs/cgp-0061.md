...
The code is verified [on CeloScan](todo:addlink), and the diff between this implementation and the current one, can be seen [here](todo:addlink).

#### 2. StableToken.mint

The two params represent who to mint to and how much:
- Who should be governance, the address can be verified
...
This address can be verified by calling `_getImplementation` on [StableTokenProxy](todo:addlink).
...
This parameters of this call are the pool address and the amount to set approval for:
- The pool address
- 5M cUSD, with 18 decimals, so 5 * 1e23 

#### 5. USDC.approve 

Same as (4) but for setting USDC approval. The [USDC contract](todo:addlink) can be verified on-chain. 
The parameters should be:
- The pool address
- 5M USDC, with 6 decimals, so 5 * 1e6 * 1e6 = 5 * 1e12

> The number of decimals for USDC can be verified [here](todo:addlink)

#### 6. Pool.addLiquidity

Adding liquidity to the pool requires thre parameters:
- The amounts to add as an array, the order of which depends on the indexes in the pool, in this case first the cUSD amount and then the USDC amount, number of decimals follows what is discussed in steps (4) and (5)
- The slippage parameters, minimum amount of LP tokens to be minted as a result of the deposit, we can safely set this to 0 in the case of a constant sum market maker.
- The deadline after which the transactions expires, this can be checked [here](todo:addlink).
