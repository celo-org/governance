...
Monday - Propose
Tuesday - Dequeued to Referendum (approval required before execution)
...
1. Run the celocli tool [governance:view](https://docs.celo.org/command-line-interface/governance#view) on a newly dequeued proposal and confirm that the `secondsUntilStages.expiration` is 777600 = 9 days (5 Referendum + 3 Execution, with approval required during these phases + 1 day buffer)
