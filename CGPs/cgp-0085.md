...
In this governance proposal, we propose to add oracles for `USDC/EUR` and `USDC/BRL`. These oracles will be used for creating `cEUR/axlUSDC` and `cREAL/axlUSDC` pools, allowing users to directly issue and redeem `cEUR` and `cREAL` against `USDC`, which will be part of the Phase 2 of the Multi-Collateral Mento upgrade (see [Mento Upgrade #1 deployment timeline](https://forum.celo.org/t/mento-upgrade-1-deployment-timeline/5219)).
